...
if (isset ($_POST['nom'])  && isset($_POST['prenom']) && isset($_POST['status']) ) {
  extract($_POST);
  $nom = $_POST['nom'];
  $prenom = $_POST['prenom'];
  ...
  $sql = 'INSERT INTO author_inventory(author_firstname, author_lastname, status_author, stays_users_id) VALUES(:nom, :prenom, :status, :username)';
  $statement = $pdo->prepare($sql);
  
  if(empty(trim($_POST['nom']))){
        $quantite_err = '<i class="fas fa-times-circle"></i>  Veuillez renseigner la quantité de l\'équipement.';
        header('refresh:1.7');
    } else if ($statement->execute([':nom' => $nom, ':prenom' => $prenom, ':status' => $status, ':username' => $username])) {
    $message = '<i class="fas fa-check-circle"></i>  Article bien enregistrer !';
    header('refresh:1.6');
  }
}
*/
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent" >
      <ul class="navbar-nav mr-auto" >
        <li class="nav-item active">
          <a class="nav-link" href="logout.php" style="color: #fff">Deconnnexion</a>
        </li>
